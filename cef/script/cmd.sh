...
python /app/automate/automate-git.py --download-dir=/app/chromium_git --depot-tools-dir=/app/depot_tools --build-target=${cef_target} ${extra_automate_args}

date=$(date +'%Y.%m.%d-%H%M%S')
archive_location="${archive_subpath}/cef-chromium-${cef_version}-${arch}-${date}.7z"
...
cd /app/chromium_git/cef/out && time 7z a -xr0'!*.git' "${archive_location}" *
